...
# @api_view(['GET'])
# def product_list_cateitem(request, slug):
#     cateitem = CategoryItem.objects.get(slug=slug)
#     serializer = CategoryItemSerializer(cateitem)
#     products= Product.objects.get(category_id=serializer["category_id"])
#     serializer1= ProductSerializer(products)
#     return Response(serializer1.data, many= True)

@api_view(['GET'])
def categoryitem_slug(request,slug):
    cateitem= CategoryItem.objects.get(slug=slug)
    serializer= CategoryItemSerializer(cateitem)
    return Response(serializer.data, status=status.HTTP_200_OK)